...
class Game(arcade.Window):
    ...
    def initgame(self):
        ...
        self.events = [
            '',
            "Ты видишь небольшой храм или святилище посреди леса. Здесь можно хорошо отдохнуть и восстановить силы.",
            # 1
    ...
    def on_draw(self):
        ...
        # Статистики игрока
        y = screen_height * 0.9
        arcade.draw_text(f"Имя: {self.username}", 20, y, COLOR_PLAYER, font_size_stats, bold=True)
        y -= screen_height * 0.05
        arcade.draw_text(f"Здоровье: {self.health}/{self.maxhealth}", 20, y, arcade.color.WHITE, font_size_stats)
        y -= screen_height * 0.05
        arcade.draw_text(f"Деньги: {self.money}", 20, y, arcade.color.WHITE, font_size_stats)
        y -= screen_height * 0.05
        arcade.draw_text(f"Урон: {self.damage}", 20, y, arcade.color.WHITE, font_size_stats)
        y -= screen_height * 0.05
        arcade.draw_text(f"Шанс крита: {self.critch}%", 20, y, arcade.color.WHITE, font_size_stats)
        y -= screen_height * 0.05
        arcade.draw_text(f"Броня: {self.armor}", 20, y, arcade.color.WHITE, font_size_stats)
        ...
        for line in self.narrative_text[-max_lines:]:
            ...
            for word in words:
                ...
                # Приблизительная проверка ширины (можно улучшить)
    ...
    def handle_continue(self):
        ...
        self.current_event = random.choice(self.events[1:])  # Исключаем пустой элемент
